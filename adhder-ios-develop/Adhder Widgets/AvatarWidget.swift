    ...
    var body: some View {
        ZStack {
            ForEach(entry.imageNames, id: \.self) { name in
                if let url = URL(string: "https://adhder-assets.s3.amazonaws.com/mobileApp/images/\(name).png") {
                    AsyncImage(url: url)
                }
                }
...
struct AvatarWidget: Widget {
    ...
    var body: some WidgetConfiguration {
        ...
        .configurationDisplayName("Adhder Avatar")
        .description("View your Adhder avatar")
