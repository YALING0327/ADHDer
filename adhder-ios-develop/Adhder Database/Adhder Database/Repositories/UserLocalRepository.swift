...
public class UserLocalRepository: BaseLocalRepository {
    ...
    public func getUserStyleWithOutfitFor(class adhderClass: AdhderClass, userID: String) -> SignalProducer<UserStyleProtocol, ReactiveSwiftRealmError> {
        return RealmUser.findBy(key: userID).map({ (user) -> UserStyleProtocol in
            ...
            let outfit = self.outfitFor(class: adhderClass)
    ...
    public func createNotification(userID: String, id: String, type: AdhderNotificationType) -> NotificationProtocol {
    ...
    private func outfitFor(class adhderClass: AdhderClass) -> OutfitProtocol {
