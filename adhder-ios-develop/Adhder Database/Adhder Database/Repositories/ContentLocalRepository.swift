...
public class ContentLocalRepository: BaseLocalRepository {
    ...
    public func save(_ worldState: WorldStateProtocol) {
        if let oldWorldState = getRealm()?.objects(RealmWorldState.self).filter("id == 'adhder'").first {
        ...
        save(object: RealmWorldState(id: "adhder", state: worldState))
    }
    
    public func getWorldState() -> SignalProducer<WorldStateProtocol, ReactiveSwiftRealmError> {
        return RealmWorldState.findBy(query: "id == 'adhder'").reactive().map({ (entry) -> WorldStateProtocol? in
