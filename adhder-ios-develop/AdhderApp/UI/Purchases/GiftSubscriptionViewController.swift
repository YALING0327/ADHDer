            ...
            VStack(spacing: 0) {
                ...
                Group {
                    if viewModel.isSubscribing {
                        ProgressView().adhderProgressStyle().frame(height: 48).transition(.opacity)
                    } else {
                        AdhderButtonUI(label: Text(L10n.giftSubscription).foregroundColor(Color(UIColor.purple100)), color: Color(UIColor.yellow100), size: .compact) {
...
class GiftSubscriptionViewController: UIHostingController<GiftSubscriptionPage> {
    ...
    func displayConfirmationDialog() {
        ...
        let alertController = AdhderAlertController(title: L10n.giftConfirmationTitle, message: body)
