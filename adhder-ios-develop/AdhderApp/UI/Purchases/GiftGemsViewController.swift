...
class GiftGemsViewController: BaseUIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
        ...
        if let user = giftedUser {
            userRepository.sendGems(amount: balanceAmount, recipient: user.id ?? "").observeValues {[weak self] response in
                if response?.success == true {
                    ...
                    let alert = AdhderAlertController.genericError(message: L10n.couldNotGiftGems)
                    alert.enqueue()
                }
            }
        }
    ...
    func showConfirmationDialog(gemCount: Int) {
        let alert = AdhderAlertController(title: L10n.giftSentConfirmation, message: L10n.giftSentTo(displayNameLabel.text ?? ""))
        let mainView = UIView()
        ...
        let iconView = UIImageView(image: AdhderIcons.imageOfGem)
