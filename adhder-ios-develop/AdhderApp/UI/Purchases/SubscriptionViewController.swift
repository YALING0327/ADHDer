...
class SubscriptionViewController: BaseTableViewController {
    ...
    let identifiers = ["subscription1month", "com.adhderapp.ios.adhder.subscription.3month",
    ...
    private var activePromo: AdhderPromotion?
    ...
    override func viewDidLoad() {
        ...
        if let termsView = self.tableView.tableFooterView?.viewWithTag(2) as? UITextView {
            ...
            termsAttributedText.addAttributes([NSAttributedString.Key.link: "https://adhder.com/static/terms"], range: termsRange)
            let privacyRange = termsAttributedText.mutableString.range(of: "Privacy Policy")
            termsAttributedText.addAttributes([NSAttributedString.Key.link: "https://adhder.com/static/privacy"], range: privacyRange)
        ...
        AdhderAnalytics.shared.logNavigationEvent("navigated subscription screen")
    }
    ...
    func subscribeToPlan() {
        ...
        SwiftyStoreKit.purchaseProduct(identifier, atomically: false) { result in
            ...
            case .error(let error):
                AdhderAnalytics.shared.log("purchase_failed", withEventProperties: ["error": error.localizedDescription, "code": error.errorCode])
