...
class ClassSelectionViewController: UIViewController, Themeable {
    ...
    private var selectedClass: AdhderClass?
    ...
    private func configure(view: ClassSelectionOptionView, class adhderClass: AdhderClass) {
        view.configure(adhderClass: adhderClass) {
            self.set(class: adhderClass)
        }
        disposable.inner.add(userRepository.getUserStyleWithOutfitFor(class: adhderClass).on(value: { userStyle in
            view.userStyle = userStyle
        }).start())
    }
    
    private func set(class adhderClass: AdhderClass) {
        self.selectedClass = adhderClass
        switch adhderClass {
        case .warrior:
        ...
        warriorOptionView.isSelected = adhderClass == .warrior
        mageOptionView.isSelected = adhderClass == .mage
        healerOptionView.isSelected = adhderClass == .healer
        rogueOptionView.isSelected = adhderClass == .rogue
    }
    ...
    override func dismiss(animated flag: Bool, completion: (() -> Void)? = nil) {
        ...
        } else if let presented = presentedViewController as? AdhderAlertController {
