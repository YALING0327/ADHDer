...
class VerifyUsernameModalViewController: UIViewController, Themeable {
    ...
    override func viewDidLoad() {
        ...
        usernameIconView.image = AdhderIcons.imageOfCheckmark(checkmarkColor: UIColor.green50, percentage: 100)
    ...
    private func displayNameChangeProducer() -> SignalProducer<Bool, Never> {
        return displaynameProperty.producer.map { name -> Bool in
            return name?.count ?? 0 > 1 && name?.count ?? 0 < 30
                ...
                if isUsable {
                    self?.displayNameIconView.image = AdhderIcons.imageOfCheckmark(checkmarkColor: UIColor.green50, percentage: 100)
                } else {
                    self?.displayNameIconView.image = AdhderIcons.imageOfAlertIcon
                }
            })
    }
        ...
        return usernameProperty.producer.throttle(2, on: QueueScheduler.main)
            ...
            .on(value: {[weak self] response in
                if response.isUsable {
                    self?.usernameIconView.image = AdhderIcons.imageOfCheckmark(checkmarkColor: UIColor.green50, percentage: 100)
                } else {
                    self?.usernameIconView.image = AdhderIcons.imageOfAlertIcon
                }
            })
    }
    ...
    private func formattedWikiString() -> NSAttributedString {
        ...
        for component in stringComponents {
            if component.starts(with: "<wk>") {
                ...
                attributedComponent.addAttribute(.link, value: "https://adhder.wikia.com/wiki/Player_Names", range: range)
    ...
    private func formattedFooterString() -> NSAttributedString {
        ...
        for component in stringComponents {
            if component.starts(with: "<ts>") {
                ...
                attributedComponent.addAttribute(.link, value: "https://adhder.com/static/terms", range: NSRange(location: 0, length: attributedComponent.length))
                finalString.append(attributedComponent)
                ...
                attributedComponent.addAttribute(.link, value: "https://adhder.com/static/community-guidelines", range: NSRange(location: 0, length: attributedComponent.length))
                finalString.append(attributedComponent)
            } else {
                let attributedComponent = NSMutableAttributedString(string: String(component))
                let range = NSRange(location: 0, length: attributedComponent.length)
