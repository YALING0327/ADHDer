...
private class ViewModel: ObservableObject {
    ...
    func useSubBenefit(_ onCompleted: @escaping () -> Void) {
        ...
        text = ""
        key = ""
        AdhderAnalytics.shared.log("Free armoire perk")
            ...
            VStack {
                ...
                HStack {
                    if viewModel.type == "gear" {
                        AdhderButtonUI(label: Text(L10n.equip), color: .white) {
                            viewModel.inventoryRepository.equip(type: "equipped", key: viewModel.key).observeCompleted {
                                onDismiss()
                            }
                        }.padding(.trailing, 16)
                    }
                    AdhderButtonUI(label: Text(L10n.close), color: .white, onTap: {
                        onDismiss()
                    })
                }
                .frame(maxWidth: 600)
                ...
                if viewModel.isSubscribed || !viewModel.enableSubBenefit {
                        ...
                        Button(action: {
                            ...
                            Group {
                                if viewModel.isUsingPerk {
                                    ProgressView().adhderProgressStyle().frame(width: 28, height: 28)
                                } else {
                    ...
                    VStack(alignment: .center, spacing: 8) {
                        AdhderButtonUI(label: Text(L10n.Armoire.unsubbedButtonPrompt).foregroundColor(Color(UIColor.teal10)), color: .white) {
