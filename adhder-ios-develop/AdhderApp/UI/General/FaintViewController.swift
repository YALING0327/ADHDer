...
struct AdhderButtonUI<Label: View>: View {
    enum Size {
        case small
        case compact
        case normal
...
private class ViewModel: ObservableObject {
    ...
    func useSubBenefit(_ onCompleted: @escaping () -> Void) {
        AdhderAnalytics.shared.log("second chance perk")
        ...
        VStack(spacing: 0) {
            ...
            ZStack {
                ForEach(0..<6, id: \.self) { index in
                    Image(uiImage: AdhderIcons.imageOfGoldReward)
            ...
            if viewModel.enableSubBenefit {
                ...
                if viewModel.isSubscribed {
                    ...
                    VStack(alignment: .center, spacing: 8) {
                        AdhderButtonUI(label: Text(L10n.Faint.unsubbedButtonPrompt).foregroundColor(Color(UIColor.teal10)), color: .white) {
