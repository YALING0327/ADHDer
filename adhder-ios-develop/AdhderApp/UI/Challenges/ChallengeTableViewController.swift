...
class ChallengeTableViewController: BaseTableViewController, UISearchBarDelegate, ChallengeFilterChangedDelegate {
    ...
    override func viewDidLoad() {
        ...
        filterButton.setImage(AdhderIcons.imageOfFilterIcon().withRenderingMode(.alwaysTemplate), for: .normal)
        ...
        self.tableView?.refreshControl = AdhderRefresControl()
    ...
    @IBAction func addChallengeAction(_ sender: Any) {
        let alert = AdhderAlertController(title: L10n.createChallenge, message: L10n.createChallengeDescription)
        alert.addAction(title: L10n.openWebsite, style: .default, isMainAction: true) { _ in
            guard let url = URL(string: "https://adhder.com/challenges/myChallenges") else {
                return
            }
