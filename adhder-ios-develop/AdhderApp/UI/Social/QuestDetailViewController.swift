...
class QuestDetailViewController: BaseUIViewController {
    ...
    private func set(quest: QuestProtocol) {
        ...
        if quest.isBossQuest {
            ...
            healthIconView.image = AdhderIcons.imageOfHeartLightBg
            bossHealthlabel.text = "\(quest.boss?.health ?? 0) HP"
            bossDifficultyImageView.image = AdhderIcons.imageOfDifficultyStars(difficulty: CGFloat(quest.boss?.strength ?? 0))
    ...
    @IBAction func cancelButtonTapped(_ sender: Any) {
        let alertController = AdhderAlertController(title: nil, message: L10n.Quests.confirmCancelInvitation)
    ...
    @IBAction func abortButtonTapped(_ sender: Any) {
        let alertController = AdhderAlertController(title: nil, message: L10n.Quests.confirmAbort)
        alertController.addAction(title: L10n.confirm, style: .destructive, isMainAction: true, handler: {[weak self] (_) in
            if let groupID = self?.groupID {
    ...
    @IBAction func forceStartButtonTapped(_ sender: Any) {
        let alertController = AdhderAlertController(title: nil, message: L10n.Quests.confirmForceStart)
    ...
    @IBAction func leaveButtonTapped(_ sender: Any) {
        let alertController = AdhderAlertController(title: nil, message: isQuestActive ? L10n.Quests.confirmLeave : L10n.Quests.confirmLeaveNostart)
        alertController.addAction(title: L10n.confirm, style: .default, isMainAction: true, handler: {[weak self] (_) in
            if let groupID = self?.groupID {
