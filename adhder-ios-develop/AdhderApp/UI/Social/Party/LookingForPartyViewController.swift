...
struct ClassLabel: View {
    ...
    private func classImage() -> UIImage? {
        switch className {
        case "warrior":
            return AdhderIcons.imageOfWarriorLightBg
        case "mage":
            return AdhderIcons.imageOfMageLightBg
        case "healer":
            return AdhderIcons.imageOfHealerLightBg
        case "rogue":
            return AdhderIcons.imageOfRogueLightBg
        default:
            return nil
        }
    }
        ...
        ScrollView {
            LazyVStack(spacing: 8) {
                ...
                if !viewModel.hasLoadedInitialData {
                    ProgressView().adhderProgressStyle(strokeWidth: 12).frame(width: 80, height: 80).padding(.top, 20)
                } else {
...
class LookingForPartyViewController: UIHostingController<LookingForPartyView> {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        AdhderAnalytics.shared.log("View Find Members", withEventProperties: [
