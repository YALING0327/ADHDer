            ...
            VStack(spacing: 0) {
                VStack(alignment: .center) {
                    ...
                    BirthdaySection(title: Text(L10n.jubilantGryphatrice)) {
                        ...
                        if viewModel.ownsGryphatrice {
                            Text(L10n.thanksForYourSupport)
                                ...
                                .padding(.vertical, 20)
                            AdhderButtonUI(label: Text(L10n.equip), color: .white, size: .compact) {
                                viewModel.equip()
                            }
                        } else {
                            ...
                            if viewModel.isPurchasing {
                                ...
                                AdhderButtonUI(label: Text(L10n.buyForX(viewModel.price)), color: .white, size: .compact) {
                                    viewModel.purchaseAsIAP()
                                }.padding(.bottom, 20)
                                AdhderButtonUI(label: HStack {
                                    Text(L10n.buyFor)
                    ...
                    BirthdaySection(title: Text(L10n.plentyOfPotions)) {
                        ...
                        PotionGrid()
                            .padding(.bottom, 12)
                        AdhderButtonUI(label: Text(L10n.visitTheMarket), color: .white, size: .compact) {
...
class BirthdayViewModel: ObservableObject {
    ...
    func purchaseWithGems() {
        isPurchasing = true
        let alert = AdhderAlertController(title: L10n.purchaseGryphatriceConfirmation(gemPrice))
