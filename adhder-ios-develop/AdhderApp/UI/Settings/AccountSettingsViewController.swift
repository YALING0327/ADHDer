...
class AccountSettingsViewController: FormViewController, Themeable, UITextFieldDelegate {
        ...
        userRepository.deleteAccount(password: password).observeValues({ response in
            DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                if response.statusCode == 200 {
                    (UIApplication.shared.delegate as? AdhderAppDelegate)?.showLoginScreen()
                } else if response.statusCode == 401 {
                    let alertView = AdhderAlertController(title: L10n.Settings.wrongPassword)
    ...
    private func showConfirmUsernameAlert() {
        let alertController = AdhderAlertController(title: L10n.Settings.confirmUsernamePrompt, message: L10n.Settings.confirmUsernameDescription)
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 12) {
                ...
                AdhderButtonUI(label: Text(L10n.Settings.resetAccount), color: Color(isValidInput() ? ThemeService.shared.theme.errorColor : ThemeService.shared.theme.dimmedColor)) {
                    onReset(text)
                }
                if !isSocial {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 12) {
                ...
                .padding(12)
                .overlay(RoundedRectangle(cornerRadius: 8).stroke().foregroundColor(Color(ThemeService.shared.theme.tableviewSeparatorColor)))
                AdhderButtonUI(label: Text(L10n.Settings.deleteAccount), color: Color(isValidInput() ? ThemeService.shared.theme.errorColor : ThemeService.shared.theme.dimmedColor)) {
                    onDelete(text)
                }
                if (!isSocial) {
                    Text(L10n.Login.forgotPassword)
