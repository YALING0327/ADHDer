...
class QuestInviteNotificationCell: BaseNotificationCell<NotificationQuestInviteProtocol> {
    ...
    func configureFor(quest: QuestProtocol) {
        attributedTitle = try? AdhderMarkdownHelper.toAdhderAttributedString(L10n.Notifications.questInvite(quest.text ?? ""))
        if (quest.boss?.health ?? 0) > 0 {
            goalTitleLabel.text = "\(L10n.defeat):"
            goalDetailLabel.text = quest.boss?.name
            difficultyDetailView.image = AdhderIcons.imageOfDifficultyStars(difficulty: CGFloat(quest.boss?.strength ?? 0))
        } else {
            ...
            difficultyDetailView.image = AdhderIcons.imageOfDifficultyStars(difficulty: 1)
        }
        setNeedsLayout()
    }
