...
class YesterdailyTaskCell: UITableViewCell {
    ...
    func configure(task: TaskProtocol) {
        ...
        titleTextView.attributedText = try? Down(markdownString: task.text?.unicodeEmoji ?? "").toAdhderAttributedString()
        ...
        for checklistItem in task.checklist {
            if let view = UIView.fromNib(nibName: "YesterdailyChecklistItem") {
                ...
                label?.attributedText = try? Down(markdownString: checklistItem.text?.unicodeEmoji ?? "").toAdhderAttributedString()
