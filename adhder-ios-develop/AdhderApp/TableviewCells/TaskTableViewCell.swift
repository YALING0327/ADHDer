...
class TaskTableViewCell: UITableViewCell, UITextViewDelegate {
    ...
    override func awakeFromNib() {
        super.awakeFromNib()
        syncErrorIndicator.image = AdhderIcons.imageOfInfoIcon(infoIconColor: UIColor.red50)
    ...
    func configure(task: TaskProtocol) {
        ...
        if let text = task.text {
            let mutableString = try? Down(markdownString: text.unicodeEmoji).toAdhderAttributedString(baseSize: 16, baseWeight: .semibold, textColor: ThemeService.shared.theme.primaryTextColor)
        ...
        if let trimmedNotes = task.notes?.trimmingCharacters(in: .whitespacesAndNewlines), trimmedNotes.isEmpty == false {
            ...
            self.subtitleLabel.attributedText = try? Down(markdownString: trimmedNotes.unicodeEmoji).toAdhderAttributedString(baseSize: 15, textColor: ThemeService.shared.theme.ternaryTextColor)
