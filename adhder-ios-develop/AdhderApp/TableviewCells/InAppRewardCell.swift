...
class InAppRewardCell: UICollectionViewCell {
        ...
        didSet {
            if isLocked {
                if availableUntil != nil {
                    infoImageView.image = AdhderIcons.imageOfItemIndicatorLocked(indicatorLocked: .purple300, lockColor: .white)
                } else {
                    if ThemeService.shared.theme.isDark {
                        infoImageView.image = AdhderIcons.imageOfItemIndicatorLockedDark()
                    } else {
                        infoImageView.image = AdhderIcons.imageOfItemIndicatorLocked()
                    }
                }
    ...
    private var availableUntil: Date? {
        didSet {
            if availableUntil != nil {
                infoImageView.image = AdhderIcons.imageOfItemIndicatorLimited()
                infoImageView.isHidden = false
                infoLabel.isHidden = true
    ...
    public var isPinned = false {
        didSet {
            ...
            if isPinned {
                pinnedIndicatorView.image = AdhderIcons.imageOfPinnedItem.withRenderingMode(.alwaysTemplate)
            }
        }
    }
    ...
    func showPurchaseConfirmation() {
        purchaseConfirmationView.image = AdhderIcons.imageOfCheckmark(checkmarkColor: .white, percentage: 1.0)
