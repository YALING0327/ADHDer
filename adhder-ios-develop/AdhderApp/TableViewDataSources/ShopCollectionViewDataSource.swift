    ...
    override func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        if let headerView = view {
            if indexPath.section == 0 && needsGearSection {
                ...
                headerView.setSecondRow(className: selectedClassName, classColor: .backgroundColorFor(adhderClass: selectedGearCategory))
                ...
                if userClass == selectedInternalGearCategory || selectedInternalGearCategory == "none" {
                    ...
                    headerView.onClassChange = {
                        ...
                        self.userRepository.selectClass(AdhderClass(rawValue: selectedClass ?? "")).observeValues { _ in
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if indexPath.section == 0 && needsGearSection && !hasGearSection() {
            ...
            if armoireCount == 0 {
                ...
                (cell.viewWithTag(8) as? UIImageView)?.image = AdhderIcons.imageOfGold
