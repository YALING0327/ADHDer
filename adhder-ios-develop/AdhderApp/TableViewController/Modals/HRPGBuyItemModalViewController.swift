...
class HRPGBuyItemModalViewController: UIViewController, Themeable {
    ...
    private var isPinned: Bool = false {
        didSet {
            if isPinned {
                ...
                pinButton.setImage(AdhderIcons.imageOfUnpinItem.withRenderingMode(.alwaysTemplate), for: .normal)
                ...
                pinButton.setImage(AdhderIcons.imageOfPinItem.withRenderingMode(.alwaysTemplate), for: .normal)
    ...
    static func displayInsufficientGemsModal(reward: InAppRewardProtocol? = nil, reason: String = "purchase modal", delayDisplay: Bool = true) {
        AdhderAnalytics.shared.log("show insufficient gems modal", withEventProperties: ["reason": "purchase modal", "item": reward?.key ?? ""])
    ...
    static func prepareInsufficientModal(title: String, message: String?, image: UIImage) -> AdhderAlertController {
        let alert = AdhderAlertController(title: title, message: message)
    ...
    func displayNoRemainingConfirmationDialog() {
        let alert = AdhderAlertController(title: L10n.excessItems, message: L10n.excessNoItemsLeft(reward?.text ?? "", purchaseQuantity, reward?.text ?? ""))
    ...
    func displaySomeRemainingConfirmationDialog(quantity: Int) {
        let alert = AdhderAlertController(title: L10n.excessItems, message: L10n.excessXItemsLeft(quantity, reward?.text ?? "", purchaseQuantity))
        alert.addAction(title: L10n.purchaseX(purchaseQuantity), isMainAction: true) { _ in
            self.buyItem(quantity: self.purchaseQuantity)
            alert.dismiss(animated: true, completion: nil)
        }
