...
class QuestCompletedAlertController: AdhderAlertController {
    ...
    init(questKey: String) {
        ...
        inventoryRepository.getQuest(key: questKey).skipNil().take(first: 1).on(value: {[weak self] quest in
            ...
            endTextView.attributedText = try? AdhderMarkdownHelper.toAdhderAttributedString(quest.completion ?? "")
