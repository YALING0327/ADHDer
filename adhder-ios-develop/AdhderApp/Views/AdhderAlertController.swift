...
@objc
class AdhderAlertController: UIViewController, Themeable {
    ...
    init() {
        super.init(nibName: "AdhderAlertController", bundle: Bundle.main)
    }
    ...
    @objc
    func enqueue() {
        AdhderAlertController.addToQueue(alert: self)
    }
    
    override func dismiss(animated flag: Bool, completion: (() -> Void)? = nil) {
        super.dismiss(animated: flag, completion: completion)
        AdhderAlertController.showNextInQueue(currentAlert: self)
    ...
    private static var alertQueue = [AdhderAlertController]()
    
    private static func showNextInQueue(currentAlert: AdhderAlertController) {
    ...
    private static func addToQueue(alert: AdhderAlertController) {
        if alertQueue.isEmpty {
            alert.show()
        }
...
extension AdhderAlertController {
    @objc
    public static func alert(title: String? = nil,
                             message: String? = nil) -> AdhderAlertController {
        let alertController = AdhderAlertController(
            title: title,
            message: message
        )
    ...
    @objc
    public static func alert(title: String? = nil,
                             attributedMessage: NSAttributedString) -> AdhderAlertController {
        let alertController = AdhderAlertController(
            title: title,
    ...
    @objc
    public static func genericError(message: String?, title: String = L10n.Errors.error) -> AdhderAlertController {
        let alertController = AdhderAlertController(
            title: title,
            message: message
        )
