...
class RageStrikeView: UIView {
    ...
    func viewTapped() {
        if isActive {
            ...
            let alertController = AdhderAlertController.alert(title: L10n.WorldBoss.rageStrikeTitle(locationName))
            ...
            attributedString.addAttribute(.font, value: UIFontMetrics.default.scaledSystemFont(ofSize: 15),
                                          range: NSRange.init(location: firstLineRange.length, length: string.count - firstLineRange.length))
            let alertController = AdhderAlertController.alert(title: L10n.WorldBoss.pendingStrike, attributedMessage: attributedString)
            alertController.addCloseAction()
            alertController.show()
            alertController.titleLabel.textColor = .white
        }
