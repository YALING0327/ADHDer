...
class QuestGoalView: UIView {
    ...
    func configure(quest: QuestProtocol) {
        ...
        if let bossHealth = quest.boss?.health, bossHealth > 0 {
            healthIcon.image = AdhderIcons.imageOfHeartDarkBg
            ...
            difficultyImageView.image = AdhderIcons.imageOfDifficultyStars(difficulty: CGFloat(quest.boss?.strength ?? 0))
        } else {
            ...
            difficultyImageView.image = AdhderIcons.imageOfDifficultyStars(difficulty: 1)
        }
    }
}
