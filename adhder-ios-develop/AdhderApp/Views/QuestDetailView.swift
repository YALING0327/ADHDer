...
class QuestDetailView: UIView, Themeable {
    ...
    func configure(quest: QuestProtocol) {
        ...
        if let experience = quest.drop?.experience, experience > 0 {
            rewardsStackView.addArrangedSubview(makeRewardView(title: L10n.Quests.rewardExperience(experience), image: AdhderIcons.imageOfExperienceReward))
        }
        if let gold = quest.drop?.gold, gold > 0 {
            rewardsStackView.addArrangedSubview(makeRewardView(title: L10n.Quests.rewardGold(gold), image: AdhderIcons.imageOfGoldReward))
        }
