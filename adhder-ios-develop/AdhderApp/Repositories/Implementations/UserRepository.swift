...
class UserRepository: BaseRepository<UserLocalRepository> {
            ...
            return UseSkillCall(key: skill.key ?? "", target: skill.target ?? "", targetID: targetId).objectSignal
                ...
                .on(value: {[weak self] values in
                    ...
                    let toastView = ToastView(title: L10n.Skills.useSkill(skill.text ?? ""),
                                              rightIcon: AdhderIcons.imageOfMagic,
                    ...
                    let newDamage = skillResponse.user?.party?.quest?.progress?.up ?? 0
                        ...
                        if newDamage > oldDamage {
                            ToastManager.show(toast: ToastView(title: L10n.Skills.causedDamage,
                                                      rightIcon: AdhderIcons.imageOfDamage,
    ...
    func runCron(checklistItems: [(TaskProtocol, ChecklistItemProtocol)], tasks: [TaskProtocol]) {
        ...
        disposable = producer.flatMap(.latest, { _ -> SignalProducer<URLResponse, Never> in
            ...
            call.serverErrorSignal.on(value: { error in
                AdhderAnalytics.shared.log("cron_failed", withEventProperties: [
    ...
    func getUserStyleWithOutfitFor(class adhderClass: AdhderClass, userID: String? = nil) -> SignalProducer<UserStyleProtocol, ReactiveSwiftRealmError> {
        return localRepository.getUserStyleWithOutfitFor(class: adhderClass, userID: userID ?? currentUserId ?? "")
    }
    ...
    func selectClass(_ adhderClass: AdhderClass? = nil) -> Signal<UserProtocol?, Never> {
        if (lastClassSelection?.timeIntervalSinceNow ?? -3) > -2 {
            return Signal.empty
        }
        lastClassSelection = Date()
        return SelectClassCall(class: adhderClass).httpResponseSignal
            .on(value: { response in
                if response.statusCode == 200 && adhderClass != nil {
                    let alert = AdhderAlertController(
                        title: L10n.classChangeSuccessTitle(adhderClass?.translatedName ?? ""),
                        message: L10n.classChangeSuccessDescription(adhderClass?.translatedName ?? ""))
    ...
    func purchaseGems(receipt: [String: Any], recipient: String? = nil) -> Signal<AdhderResponse<APIEmptyResponse>?, Never> {
        return PurchaseGemsCall(receipt: receipt, recipient: recipient).adhderResponseSignal
    }
    
    func sendGems(amount: Int, recipient: String) -> Signal<AdhderResponse<APIEmptyResponse>?, Never> {
        return SendGemsCall(amount: amount, recipient: recipient).adhderResponseSignal
    }
    ...
    func createNotification(id: String, type: AdhderNotificationType) -> NotificationProtocol {
