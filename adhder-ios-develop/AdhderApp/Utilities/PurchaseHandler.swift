...
class PurchaseHandler: NSObject, SKPaymentTransactionObserver {
    ...
    static let IAPIdentifiers = ["com.adhderapp.ios.Adhder.4gems", "com.adhderapp.ios.Adhder.21gems",
                              "com.adhderapp.ios.Adhder.42gems", "com.adhderapp.ios.Adhder.84gems"
    ]
    static let subscriptionIdentifiers = ["subscription1month", "com.adhderapp.ios.adhder.subscription.3month",
                       "com.adhderapp.ios.adhder.subscription.6month", "com.adhderapp.ios.adhder.subscription.12month"
    ]
    static let noRenewSubscriptionIdentifiers = ["com.adhderapp.ios.adhder.norenew_subscription.1month", "com.adhderapp.ios.adhder.norenew_subscription.3month",
                                          "com.adhderapp.ios.adhder.norenew_subscription.6month", "com.adhderapp.ios.adhder.norenew_subscription.12month"
    ]
    
    static let gryphatriceIdentifier = "com.adhderapp.ios.Adhder.pets.Gryphatrice_Jubilant"
    
    static let adhderSubMapping = [
        "subscription1month": "basic_earned",
        "com.adhderapp.ios.adhder.subscription.3month": "basic_3mo",
        "com.adhderapp.ios.adhder.subscription.6month": "basic_6mo",
        "com.adhderapp.ios.adhder.subscription.12month": "basic_12mo"
    ]
    
    static let adhderSubMappingReversed = [
        "basic_earned": "subscription1month",
        "basic_3mo": "com.adhderapp.ios.adhder.subscription.3month",
        "basic_6mo": "com.adhderapp.ios.adhder.subscription.6month",
        "basic_12mo": "com.adhderapp.ios.adhder.subscription.12month"
    ]
            ...
            if self.isInAppPurchase(productIdentifier) {
                ...
                if !user.isSubscribed || user.purchased?.subscriptionPlan?.dateCreated == nil ||
                    ...
                    (user.purchased?.subscriptionPlan?.customerId == transaction.original?.transactionIdentifier
                     ...
                     && PurchaseHandler.adhderSubMapping[transaction.payment.productIdentifier] != user.purchased?.subscriptionPlan?.planId) {
    ...
    func isAllowedToMakePurchases() -> Bool {
        ...
        if AdhderAppDelegate.isRunningLive() || testinglevel.isTrustworthy {
            return true
        }
        return false
        #endif
    ...
    private func checkForCancellation(user: UserProtocol) {
        ...
        SwiftyStoreKit.verifyReceipt(using: self.appleValidator) { result in
            switch result {
            case .success(let receipt):
                let sku = PurchaseHandler.adhderSubMappingReversed[user.purchased?.subscriptionPlan?.planId ?? ""] ?? ""
