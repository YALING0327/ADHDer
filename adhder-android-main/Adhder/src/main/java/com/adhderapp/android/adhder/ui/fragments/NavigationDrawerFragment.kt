package com.adhderapp.android.adhder.ui.fragments
...
import com.adhderapp.android.adhder.databinding.DrawerMainBinding
import com.adhderapp.android.adhder.extensions.consumeWindowInsetsAbove30
import com.adhderapp.android.adhder.extensions.getMinuteOrSeconds
import com.adhderapp.android.adhder.extensions.getRemainingString
...
import com.adhderapp.android.adhder.models.promotions.PromoType
...
import com.adhderapp.android.adhder.ui.activities.NotificationsActivity
import com.adhderapp.android.adhder.ui.adapter.NavigationDrawerAdapter
import com.adhderapp.android.adhder.ui.menu.AdhderDrawerItem
...
class NavigationDrawerFragment : DialogFragment() {
    ...
    private var activePromo: AdhderPromotion? = null
            ...
            contentRepository.getWorldState()
                ...
                .collect { (worldState, items) ->
                    ...
                    if (event != null && item == null) {
                        ...
                        val birthdayItem = AdhderDrawerItem(R.id.birthdayActivity, SIDEBAR_BIRTHDAY)
                        birthdayItem.itemViewType = 6
                        val newItems = mutableListOf<AdhderDrawerItem>()
    ...
    private fun updateUser(user: User) {
        ...
        if (item != null) {
            if (!user.hasClass && !hasSpecialItems) {
                ...
                if ((
                    user.stats?.lvl
                        ?: 0
                    ) < AdhderSnackbar.MIN_LEVEL_FOR_SKILLS && (!hasSpecialItems)
                ) {
        ...
        if (user.isSubscribed && user.purchased?.plan?.dateTerminated != null) {
            ...
            subscriptionItem?.subtitle = context?.getString(R.string.more_out_of_adhder)
        }
    ...
    private fun initializeMenuItems() {
        val items = ArrayList<AdhderDrawerItem>()
        context?.let { context ->
            items.add(
                AdhderDrawerItem(
                    R.id.tasksFragment,
            ...
            val customizationShopItem = AdhderDrawerItem(
        ...
        val promoItem = AdhderDrawerItem(R.id.subscriptionPurchaseActivity, SIDEBAR_PROMO)
        ...
        if (configManager.showSubscriptionBanner()) {
            val item =
                AdhderDrawerItem(R.id.subscriptionPurchaseActivity, SIDEBAR_SUBSCRIPTION_PROMO)
            item.itemViewType = 2
            items.add(item)
        }
    ...
    private fun getItemWithIdentifier(identifier: String): AdhderDrawerItem? =
    ...
    fun updatePromo() {
        ...
        activePromo?.let { activePromo ->
            ...
            var promotedItem: AdhderDrawerItem? = null
