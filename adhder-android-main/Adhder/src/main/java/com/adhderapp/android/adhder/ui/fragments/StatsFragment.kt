...
import com.adhderapp.android.adhder.databinding.FragmentStatsBinding
...
import com.adhderapp.android.adhder.ui.views.stats.BulkAllocateStatsDialog
...
class StatsFragment : BaseMainFragment<FragmentStatsBinding>() {
    ...
    override fun onViewCreated(
        ...
        binding?.leftSparklesView?.setImageBitmap(AdhderIconsHelper.imageOfAttributeSparklesLeft())
        binding?.rightSparklesView?.setImageBitmap(AdhderIconsHelper.imageOfAttributeSparklesRight())
        context?.let {
            val color = it.getThemeColor(R.attr.colorPrimaryOffset)
            binding?.distributeEvenlyHelpButton?.setImageBitmap(
                AdhderIconsHelper.imageOfInfoIcon(
                    color
                )
            )
    ...
    private fun showHelpAlert(resourceId: Int) {
        val alert = context?.let { AdhderAlertDialog(it) }
