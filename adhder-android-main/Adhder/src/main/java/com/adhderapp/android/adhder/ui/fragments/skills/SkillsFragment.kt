package com.adhderapp.android.adhder.ui.fragments.skills
...
import com.adhderapp.android.adhder.databinding.FragmentRecyclerviewBinding
...
import com.adhderapp.android.adhder.ui.adapter.SkillsRecyclerViewAdapter
...
import com.adhderapp.android.adhder.ui.views.AdhderSnackbar.Companion.showSnackbar
...
class SkillsFragment : BaseMainFragment<FragmentRecyclerviewBinding>() {
    ...
    private fun onSkillSelected(skill: Skill) {
        ...
        val resourceIconDrawable: Drawable = AdhderIconsHelper.imageOfMagic().toDrawable(context.resources)
    ...
    private fun displaySkillResult(
        ...
        if ("special" == usedSkill?.habitClass) {
            showSnackbar(
                ...
                AdhderSnackbar.SnackbarDisplayType.BLUE
            )
        } else {
            context?.let {
                showSnackbar(
                    ...
                    null,
                    context?.getString(R.string.used_skill_without_mana, usedSkill?.text),
                    BitmapDrawable(resources, AdhderIconsHelper.imageOfMagic()),
        ...
        if (response.damage > 0) {
            lifecycleScope.launch(ExceptionHandler.coroutine()) {
                ...
                showSnackbar(
                    ...
                    BitmapDrawable(resources, AdhderIconsHelper.imageOfDamage()),
