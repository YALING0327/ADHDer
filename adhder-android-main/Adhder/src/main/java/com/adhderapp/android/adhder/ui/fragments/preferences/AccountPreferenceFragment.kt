...
import com.adhderapp.android.adhder.extensions.addOkButton
import com.adhderapp.android.adhder.models.user.User
import com.adhderapp.android.adhder.ui.activities.FixCharacterValuesActivity
import com.adhderapp.android.adhder.ui.fragments.preferences.AdhderAccountDialog.AccountUpdateConfirmed
import com.adhderapp.android.adhder.ui.helpers.KeyboardUtil
import com.adhderapp.android.adhder.ui.viewmodels.AuthenticationViewModel
import com.adhderapp.android.adhder.ui.views.AboutMeScreen
import com.adhderapp.android.adhder.ui.views.ApiTokenBottomSheet
import com.adhderapp.android.adhder.ui.views.ChangeDisplayNameScreen
import com.adhderapp.android.adhder.ui.views.ChangeEmailScreen
import com.adhderapp.android.adhder.ui.views.ChangePasswordScreen
import com.adhderapp.android.adhder.ui.views.ChangeUsernameScreen
import com.adhderapp.android.adhder.ui.views.ExtraLabelPreference
import com.adhderapp.android.adhder.ui.views.AdhderSnackbar
import com.adhderapp.android.adhder.ui.views.PhotoUrlScreen
import com.adhderapp.android.adhder.ui.views.SnackbarActivity
import com.adhderapp.android.adhder.ui.views.ValidatingEditText
import com.adhderapp.android.adhder.ui.views.dialogs.AdhderAlertDialog
import com.adhderapp.android.adhder.ui.views.dialogs.AdhderProgressDialog
...
class AccountPreferenceFragment :
    ...
    private lateinit var accountDialog: AdhderAccountDialog
    ...
    private fun disconnect(
        ...
        context?.let { context ->
            val dialog = AdhderAlertDialog(context)
    ...
    private fun showForgotPasswordDialog() {
        ...
        val alertDialog = AdhderAlertDialog(requireContext())
    ...
    private fun showPasswordEmailConfirmation() {
        val alert = AdhderAlertDialog(requireContext())
    ...
    private fun showAccountDeleteConfirmation(user: User?) {
        if (user?.purchased?.plan?.isActive == true && user.purchased?.plan?.dateTerminated == null) {
            val dialog = context?.let { AdhderAlertDialog(it) }
        ...
        val adhderAccountDialog = context?.let { AdhderAccountDialog(it) }
        adhderAccountDialog?.accountAction = "delete_account"
        adhderAccountDialog?.accountUpdateConfirmed = this
        adhderAccountDialog?.user = user
        adhderAccountDialog?.show(childFragmentManager, AdhderAccountDialog.TAG)

        if (adhderAccountDialog != null) {
            accountDialog = adhderAccountDialog
        }
    }

    private fun deleteAccount(password: String) {
        val dialog = activity?.let { AdhderProgressDialog.show(it, R.string.deleting_account) }
        lifecycleScope.launchCatching({ throwable ->
            dialog?.dismiss()
            if (throwable is HttpException && throwable.code() == 401) {
                val errorDialog = context?.let { AdhderAlertDialog(it) }
            ...
            context?.let {
                ...
                AdhderBaseApplication.logout(it, user)
            }
            activity?.finish()
        }
    }
    ...
    private fun showAccountResetConfirmation(user: User?) {
        val adhderAccountDialog = context?.let { AdhderAccountDialog(it) }
        adhderAccountDialog?.accountAction = "reset_account"
        adhderAccountDialog?.accountUpdateConfirmed = this
        adhderAccountDialog?.user = user
        adhderAccountDialog?.show(parentFragmentManager, "account")
    ...
    private fun resetAccount(confirmationString: String) {
        val progressDialog = activity?.let { AdhderProgressDialog.show(it, R.string.resetting_account) }
        lifecycleScope.launch(ExceptionHandler.coroutine()) {
