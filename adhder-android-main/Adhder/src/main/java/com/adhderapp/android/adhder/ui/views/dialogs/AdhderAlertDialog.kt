...
import com.adhderapp.android.adhder.databinding.DialogAdhderBaseBinding
...
open class AdhderAlertDialog(context: Context) : AlertDialog(context, R.style.AdhderAlertDialogTheme) {
    ...
    private val binding = DialogAdhderBaseBinding.inflate(LayoutInflater.from(context))
    val scrollView
    ...
    fun addButton(
        ...
        function: ((AdhderAlertDialog, Int) -> Unit)? = null
    ): Button {
    ...
    fun addButton(
        ...
        val button: Button =
            if (isPrimary) {
                if (isDestructive) {
                    binding.buttonsWrapper.inflate(R.layout.dialog_adhder_primary_destructive_button) as? Button
                } else {
                    binding.buttonsWrapper.inflate(R.layout.dialog_adhder_primary_button) as? Button
                }
            } else {
                val button = binding.buttonsWrapper.inflate(R.layout.dialog_adhder_secondary_button) as? Button
                if (isDestructive) {
    ...
    companion object {
        private var dialogQueue = mutableListOf<AdhderAlertDialog>()

        private fun showNextInQueue(currentDialog: AdhderAlertDialog) {
        ...
        private fun addToQueue(dialog: AdhderAlertDialog) {
            try {
