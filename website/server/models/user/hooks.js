...
function _populateDefaultTasks (user, taskTypes) {
  let defaultsData;
  if (user.registeredThrough === 'adhder-android' || user.registeredThrough === 'adhder-ios') {
  ...
  const tasksToCreate = [];
  if (user.registeredThrough === 'adhder-web') return Promise.all(tasksToCreate);
...
function _setUpNewUser (user) {
  ...
  if (user.registeredThrough === 'adhder-web') {
...
schema.post('init', function postInitUser () {
  ...
  // In particular:
  // - tags https://github.com/AdhderApp/adhder/issues/10688
  // - notifications https://github.com/AdhderApp/adhder/issues/9923
  // - push devices https://github.com/AdhderApp/adhder/issues/11805
  //            and https://github.com/AdhderApp/adhder/issues/11868
