...
export async function createChallenge (user, req, res) {
  ...
  categories.forEach(category => {
    if (category.slug === 'adhder_official' && !user.hasPermission('challengeAdmin')) {
      throw new NotAuthorized(res.t('noPrivAccess'));
    } else if (category.slug === 'adhder_official' && user.hasPermission('challengeAdmin')) {
