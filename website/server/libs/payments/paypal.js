...
api.constants = {
  // CURRENCY_CODE: 'USD',
  // SELLER_NOTE: 'Adhder Payment',
  // SELLER_NOTE_SUBSCRIPTION: 'Adhder Subscription',
  // SELLER_NOTE_ATHORIZATION_SUBSCRIPTION: 'Adhder Subscription Payment',
  // STORE_NAME: 'Adhder',
  //
  // GIFT_TYPE_GEMS: 'gems',
...
api.checkout = async function checkout (options = {}) {
  ...
  if (gift) {
    ...
    if (gift.type === 'gems') {
      ...
      description = 'mo. Adhder Subscription (Gift)';
    }
  } else if (sku) {
...
api.subscribe = async function subscribe (options = {}) {
  ...
  const billingPlanTitle = `Adhder Subscription ($${sub.price} every ${sub.months} months, recurring)`;
