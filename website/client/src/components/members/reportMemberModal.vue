...
export default {
  ...
  mounted () {
    this.$root.$on('adhder::report-profile', this.handleReport);
  },
  beforeDestroy () {
    this.$root.$off('adhder::report-profile', this.handleReport);
  },
  methods: {
    close () {
    ...
    async reportAbuse () {
      ...
      if (result.status === 200) {
        ...
        this.$root.$emit('adhder:report-profile-result', result.data.data);
      } else {
