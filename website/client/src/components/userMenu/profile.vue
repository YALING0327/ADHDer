...
const COMMUNITY_MANAGER_EMAIL = 'admin@adhder.com';
...
export default {
  ...
  mounted () {
    ...
    this.$root.$on('adhder:report-profile-result', () => {
      this.loadUser();
    });
  },
  updated () {
  ...
  beforeDestroy () {
    ...
    this.$root.$off('adhder:report-profile-result');
  },
  methods: {
    async loadUser () {
      let user = null;
    ...
    openSendGemsModal () {
      ...
      this.$root.$emit('adhder::send-gift', this.user);
    },
    ...
    reportPlayer () {
      this.$root.$emit('adhder::report-profile', {
