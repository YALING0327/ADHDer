...
export default {
  ...
  async mounted () {
    ...
    this.isOfficial = this.challenge.official
      || this.challenge.categories?.some(category => category.name === 'adhder_official');
  ...
  methods: {
    ...
    showMemberModal () {
      ...
      this.$root.$emit('adhder:show-member-modal', {
    ...
    edit () {
      this.$root.$emit('adhder:update-challenge', {
    ...
    openMemberProgressModal (member) {
      this.$root.$emit('adhder:challenge:member-progress', {
    ...
    cloneChallenge () {
      this.$root.$emit('adhder:clone-challenge', {
        challenge: this.challenge,
      });
    },
    reportChallenge () {
      this.$root.$emit('adhder::report-challenge', {
        challenge: this.challenge,
      });
    },
