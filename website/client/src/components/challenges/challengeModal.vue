      ...
      >
        ...
        >
          ...
          <div
            ...
            v-if="group.key !== 'adhder_official' || hasPermission(user, 'challengeAdmin')"
            :key="group.key"
            class="form-check"
          >
  ...
  strong(v-once) {{$t('endDate')}}
      ...
      <div class="row footer-wrap">
        <div class="col-12 text-center submit-button-wrapper">
          ...
          <!-- see https://github.com/AdhderApp/adhder/blob/develop/website/views/options/social/challenges.jade#L134-->
          <button
...
export default {
  ...
  mounted () {
    this.$root.$on('adhder:clone-challenge', data => {
    ...
    this.$root.$on('adhder:update-challenge', data => {
      if (!data.challenge) return;
      this.cloning = false;
      this.$store.state.challengeOptions.workingChallenge = {
        ...this.$store.state.challengeOptions.workingChallenge,
    ...
    this.$root.$on('adhder:create-challenge', () => {
      this.cloning = false;
      this.$store.state.challengeOptions.workingChallenge = {};
      this.$root.$emit('bv::show::modal', 'challenge-modal');
    });
  ...
  beforeDestroy () {
    this.$root.$off('adhder:clone-challenge');
    this.$root.$off('adhder:update-challenge');
    this.$root.$off('adhder:create-challenge');
  },
  methods: {
    async shown () {
