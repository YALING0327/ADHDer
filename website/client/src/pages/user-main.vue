...
export default {
  ...
  created () {
    this.$root.$on('playSound', sound => {
      ...
      const file = `https://adhder-assets.s3.amazonaws.com/mobileApp/sounds/${theme}/${sound}`;
    ...
    Promise.all([
      ...
      if (window && window['adhder-i18n']) {
        if (this.user.preferences.language === window['adhder-i18n'].language.code) {
          return null;
        }
      }
      if (window && window['adhder-i18n']) {
      ...
      const i18nData = window && window['adhder-i18n'];
      ...
      if (appState) {
        ...
        if (appState.paymentCompleted) {
          ...
          this.$root.$emit('adhder:payment-success', appState);
        }
      }
      this.$nextTick(() => {
