...
describe('User Model', () => {
  ...
  describe('validates email', () => {
    ...
    it('throws an error if email is using a restricted domain', () => {
      const user = new User();
      user.auth.local.email = 'scammer@adhder.com';
      const errors = user.validateSync();
      expect(errors.errors['auth.local.email'].message).to.equal(common.i18n.t('invalidEmailDomain', { domains: 'adhder.com, adhderapp.com' }));
    });
