...
describe('highlightMentions', () => {
  ...
  describe('link interactions', async () => {
    ...
    // https://github.com/AdhderApp/adhder/issues/12217
    it('doesn\'t highlight user in link with url-escapable characters', async () => {
      const text = '[test](https://adhder.fandom.com/ru/@wiki/Снаряжение)';
      const result = await highlightMentions(text);
      expect(result[0]).to.equal(text);
    });

    // https://github.com/AdhderApp/adhder/issues/12223
    it('matches a link in between two the same links', async () => {
      const text = '[here](http://adhder.wikia.com/wiki/The_Keep:Pirate_Cove/FAQ)\n@user\n[hier](http://adhder.wikia.com/wiki/The_Keep:Pirate_Cove/FAQ)';

      const result = await highlightMentions(text);

      expect(result[0]).to.equal('[here](http://adhder.wikia.com/wiki/The_Keep:Pirate_Cove/FAQ)\n[@user](/profile/111)\n[hier](http://adhder.wikia.com/wiki/The_Keep:Pirate_Cove/FAQ)');
    });
  });
  ...
  describe('base url', () => {
    it('should prefix adhder.com if it is production', async () => {
      ...
      expect(result[0]).to.equal('[@user](https://adhder.com/profile/111)');
